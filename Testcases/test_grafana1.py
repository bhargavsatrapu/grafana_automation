...
def test_grafana_application_1(setup):
    testDataFile = open("./Test_data/test1.json")
    ...
    login_page.login_in_grafana(ReadConfig.get_config_properties("USER_NAME"),ReadConfig.get_config_properties("PASSWORD"))
    grafana_page.time_series_visualization(data["query_select_data"],data["path_generated_graph_image"])
    assert image_comparison_util.assert_images(data["path_expected_graph_image"],data["path_generated_graph_image"])==1,"both graph are not identical"
